...
class BranchWiseDiscountExport {
...
// Footer widget for session information (SessionController number and name)
    ...
    for (int i = 0;
        i <
            controller
                .branchWiseDiscountList.value.body!.branchesDiscount!.length;
        i++) {
      final branch =
          controller.branchWiseDiscountList.value.body!.branchesDiscount![i];
      rows.add([
        "${i + 1}",
        (branch.branch?.name ?? 'Null'),
        (branch.discount.toString() ?? 'Null'),
        (branch.status == 0
            ? 'pending'
            : branch.status == 1
                ? 'Approved'

...
import '../../../../../viewmodel/admin/expense/expense_detail_viewmodel.dart';
import '../../../../../viewmodel/user_preference/session_controller.dart';

class ExpenseDetailExport {
...
// Footer widget for session information (SessionController number and name)
    ...
    for (int i = 0;
        i < controller.expenseDetailList.value.body!.generalExpenses!.length;
        i++) {
      final expense =
          controller.expenseDetailList.value.body!.generalExpenses![i];
      rows.add([
        "${i + 1}",
        (expense.expense?.name ?? 'Null'),
        (expense.description?.toString() ?? 'Null'),
        (expense.amount?.toString() ?? 'Null'),
        (expense.date?.toString() ?? 'Null'),
      ]);
    }

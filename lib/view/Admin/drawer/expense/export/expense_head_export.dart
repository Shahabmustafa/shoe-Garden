...
import '../../../../../viewmodel/admin/expense/expense_head_viewmodel.dart';
import '../../../../../viewmodel/user_preference/session_controller.dart';

class ExpenseHeadExport {
...
// Footer widget for session information (SessionController number and name)
    ...
    for (int i = 0;
        i < controller.expenseList.value.body!.expenses!.length;
        i++) {
      final expense = controller.expenseList.value.body!.expenses![i];
      rows.add([
        "${i + 1}",
        (expense.name ?? 'Null'),
      ]);

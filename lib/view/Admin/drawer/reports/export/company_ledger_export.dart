...
import '../../../../../viewmodel/report/company_customer_ladger.dart';
import '../../../../../viewmodel/user_preference/session_controller.dart';

class CompanyLedgerExport {
  ...
  Future<Uint8List> generatePdf() async {
    ...
    for (int i = 0; i < controller.ledgerCompanyList.value.body!.companies!.length; i++) {
      final company = controller.ledgerCompanyList.value.body!.companies![i];
      rows.add([
        "${i + 1}",
        (company.name ?? 'Null'),
        (company.email ?? 'Null'),
        ...
        (company.firstBalance?.toString() ?? 'Null'),
        (company.openingBalance?.toString() ?? 'Null'),
      ]);
    }
...
// Split rows into pages
    for (int start = 0; start < rows.length; start += rowsPerPage) {
      final rowsForPage = rows.sublist(start, (start + rowsPerPage > rows.length) ? rows.length : start + rowsPerPage);
      pdf.addPage(
        pw.Page(

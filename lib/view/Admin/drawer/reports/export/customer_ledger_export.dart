...
// Footer widget for session information (SessionController number and name)
    ...
    for (int i = 0; i < controller.ledgerCustomerList.value.body!.customers!.length; i++) {
      final customerr = controller.ledgerCustomerList.value.body!.customers![i];
      rows.add([
        "${i + 1}",
        (customerr.name ?? 'Null'),
        (customerr.email ?? 'Null'),
        (customerr.phoneNumber ?? 'Null'),
        (customerr.address ?? 'Null'),
        (customerr.firstBalance?.toString() ?? 'Null'),
        (customerr.openingBalance?.toString() ?? 'Null'),
      ]);
    }

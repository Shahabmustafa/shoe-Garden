...
import '../../../../../viewmodel/branch/report/sale_expense_gross_viewmodel.dart';
import '../../../../../viewmodel/user_preference/session_controller.dart';

class AdminCashCounterExport {
...
// Footer widget for session information (SessionController number and name)
    ...
    for (int i = 0;
    i < controller.BSaleExpenseGrossList.value.body!.length;
    i++) {
      final cash =
      controller.BSaleExpenseGrossList.value.body![i];
      rows.add([
        "${i + 1}",
        (cash.date ?? 'Null'),
        (cash.cashSale.toString() ?? 'Null'),
        (cash.cardPayment.toString() ?? 'Null'),
        (cash.sale.toString() ?? 'Null'),
        (cash.expense.toString() ?? 'Null'),
        (cash.gross.toString() ?? 'Null'),
        (cash.cashPaidToAdmin.toString() ?? 'Null'),
        (cash.difference.toString() ?? 'Null'),
      ]);
    }

...
import '../../../../../viewmodel/report/profit_loss_viewmodel.dart';
import '../../../../../viewmodel/user_preference/session_controller.dart';

class ProfitAndLossExport {
...
// Footer widget for session information (SessionController number and name)
    ...
    for (int i = 0;
        i < controller.profitOrLossList.value.body!.products!.length;
        i++) {
      final item = controller.profitOrLossList.value.body!.products![i];
      rows.add([
        "${i + 1}",
        (item.product != null ? item.product!.name.toString() : "Null"),
        (item.company != null ? item.company!.name.toString() : "Null"),
        (item.brand != null ? item.brand!.name.toString() : "Null"),
        (item.type != null ? item.type!.name.toString() : "Null"),
        (item.color != null ? item.color!.name.toString() : "Null"),
        (item.size != null ? item.size!.number.toString() : "Null"),
        (item.saleInvoiceId != null ? item.saleInvoiceId!.toString() : "Null"),
        (item.salePrice != null ? item.salePrice!.toString() : "Null"),
        (item.purchasePrice != null ? item.purchasePrice!.toString() : "Null"),
        (item.quantity != null ? item.quantity!.toString() : "Null"),
        (item.subTotal != null ? item.subTotal!.toString() : "Null"),
        (item.discount != null ? item.discount.toString() : "Null"),

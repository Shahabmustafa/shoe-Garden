...
import '../../../../../viewmodel/admin/salaries/salaries_viewmodel.dart';
import '../../../../../viewmodel/user_preference/session_controller.dart';

class ViewSalariesExport {
  final controller = Get.put(SalemenSalariesViewModel());
...
// Footer widget for session information (SessionController number and name)
    ...
    for (int i = 0;
        i < controller.salariesList.value.body!.saleMenSalaries!.length;
        i++) {
      final saleman = controller.salariesList.value.body!.saleMenSalaries![i];

      var branchName = controller.findBranchName(saleman.branchId.toString());
      rows.add([
        "${i + 1}",
        (saleman.name ?? 'Null'),
        (branchName.toString()),
        (saleman.phoneNumber ?? 'Null'),
        (saleman.address ?? 'Null'),
        (saleman.salary.toString() ?? 'Null'),
        (saleman.totalSale != null ? saleman.totalSale.toString() : "null"),
        (saleman.commission != null ? saleman.commission.toString() : "null"),
        (saleman.netSalary != null ? saleman.netSalary.toString() : "null"),
      ]);
    }

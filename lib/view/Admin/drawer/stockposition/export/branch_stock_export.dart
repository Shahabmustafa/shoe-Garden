  ...
  Future<Uint8List> generatePdf() async {
    ...
    final List<pw.Widget> headers = [
      ...
      'Remaining Quantity',
        ...
        padding: const pw.EdgeInsets.all(5))).toList();

    /// Create a list to hold table rows
    final List<List<String>> rows = [];
    for (int i = 0; i < controller.branchStockList.value.body!.branchStocks!.length; i++) {
      final stock = controller.branchStockList.value.body!.branchStocks![i];
      rows.add([
        "${i + 1}",
        (stock.branch?.name?.toString() ?? 'Null'),
        (stock.product?.name?.toString() ?? 'Null'),
        (stock.product?.salePrice?.toString() ?? "Null"),

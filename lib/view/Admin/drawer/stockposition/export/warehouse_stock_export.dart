...
import 'package:sofi_shoes/viewmodel/admin/stockposition/warehouse_stock_viewmodel.dart';

import '../../../../../viewmodel/user_preference/session_controller.dart';

class WarehouseStockExport {
  final controller = Get.put(WarehouseStockViewModel());
...
// Footer widget for session information (SessionController number and name)
    ...
    for (int i = 0;
        i < controller.warehouseStockList.value.body!.warehouseStocks!.length;
        i++) {
      final warehouse =
          controller.warehouseStockList.value.body!.warehouseStocks![i];

      rows.add([
        "${i + 1}",
        (warehouse.warehouse?.name ?? 'Null'),
        (warehouse.brand?.name ?? "null"),
        (warehouse.product?.name ?? 'Null'),
        (warehouse.color?.name ?? 'Null'),
        (warehouse.size?.number.toString() ?? 'Null'),
        (warehouse.quantity?.toString() ?? 'Null'),
        (warehouse.remainingQuantity?.toString() ?? 'Null'),
        (warehouse.totalPurchasePrice?.toString() ?? 'Null'),
        (warehouse.totalSalePrice?.toString() ?? 'Null'),
      ]);
    }

...
// Footer widget for session information (SessionController number and name)
    ...
    for (int i = 0;
        i < controller.warehouseClaimList.value.body!.claims!.length;
        i++) {
      final claim = controller.warehouseClaimList.value.body!.claims![i];
      rows.add([
        "${i + 1}",
        (claim.claimTo?.name ?? 'Null'),

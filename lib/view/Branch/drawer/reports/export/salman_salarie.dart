...
import '../../../../../viewmodel/branch/salaries/salman_salaries_repository.dart';
import '../../../../../viewmodel/user_preference/session_controller.dart';

class SalemanSalariesExport {
  final controller = Get.put(BranchSalmanSalariesViewModel());
...
// Footer widget for session information (SessionController number and name)
    ...
    for (int i = 0;
        i < controller.salesmanSalariesList.value.body!.saleMenSalaries!.length;
        i++) {
      final saleman =
          controller.salesmanSalariesList.value.body!.saleMenSalaries![i];
      rows.add([
        "${i + 1}",
        (saleman.name ?? 'Null'),
        (saleman.salary.toString() ?? 'Null'),
        (saleman.address.toString()),
        (saleman.phoneNumber.toString()),

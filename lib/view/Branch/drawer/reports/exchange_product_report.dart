...
import 'package:sofi_shoes/viewmodel/branch/saleinvoice/exchange_product_viewmodel.dart';
...
class BExchangeProductReport extends StatefulWidget {
  const BExchangeProductReport({super.key});

  @override
  State<BExchangeProductReport> createState() => _BExchangeProductReportState();
}

class _BExchangeProductReportState extends State<BExchangeProductReport> {
                          ...
                          child: ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              final invoiceData = InvoiceExport(
                                id: exchange.invoiceNumber?.toString() ??
                                    'Unknown',
                                paymentMethod: exchange.paymentMethod ?? "",
                                date: DateTime.now(),
                                saleman: Salesman(
                                  name: controller.getSalesmanName(
                                          exchange.newSaleMenId?.toString() ??
                                              '') ??
                                      'Unknown',
                                ),
                                customer: Customer(
                              ...
                              if (controller.search.value.text.isEmpty ||
                                  ...
                                  exchange.invoiceNumber
                                          ...
                                          .contains(controller.search.value.text
                                              ...
                                              .toLowerCase()) ==
                                      true) {
                                String saleman = controller.getSalesmanName(
                                        exchange.newSaleMenId?.toString() ??
                                            '') ??
                                    'Unknown';
                                                          ...
                                                          children: [
                                                            SingleChildScrollView(
                                                              ...
                                                              child: DataTable(
                                                                ...
                                                                rows: exchange.exchangesProducts?.map((product) {
                                                                      return DataRow(
                                                                        cells: [
                                                                          DataCell(Text(product.newProduct?.name?.toString() ?? "Null")),
                                                                          DataCell(Text(product.newColor?.name?.toString() ?? "Null")),
                                                                          DataCell(Text(product.newSize?.number?.toString() ?? "Null")),
                                                                          DataCell(Text(product.newQuantity?.toString() ?? "Null")),
                                                                          DataCell(Text(product.newProduct?.salePrice?.toString() ?? "Null")),
                                                                          DataCell(Text(product.newSubTotal?.toString() ?? 'null')),
                                                                          DataCell(Text(product.newDiscount != null ? "${product.newDiscount}%" : "Null")),
                                                                          DataCell(Text(product.newTotalAmount?.toString() ?? 'null')),
                                                                        ],
                                                                      );
                                                                    }).toList() ??
                                                                    [],
                                                            ...
                                                            Row(
                                                              ...
                                                              children: [
                                                                ...
                                                                const SizedBox(
                                                                    width: 30),
                                                                Text(
                                                                  exchange.receivedAmount?.toString() ?? "Null",
                                                                  style:
                                                                  GoogleFonts.lato(
                                                                    fontSize: 14,

...
// Footer widget for session information (SessionController number and name)
    ...
    for (int i = 0;
        i < controller.branchTargetList.value.body!.dateList!.length;
        i++) {
      final target = controller.branchTargetList.value.body!.dateList![i];
      rows.add([
        "${i + 1}",
        (target.targetAmount?.toString() ?? 'Null'),
        (target.tatalSale?.toString() ?? 'Null'),
        (target.date?.toString() ?? "Null"),
      ]);
    }
...
// Split rows into pages
  ...
  Future<void> printTargetPdf() async {
    // Generate the PDF

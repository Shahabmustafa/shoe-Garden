...
import '../../../../../viewmodel/branch/product/return_product_to_warehouse_view_model.dart';
import '../../../../../viewmodel/user_preference/session_controller.dart';

class ReturnProductWarehouseExport {
  ...
  Future<Uint8List> generatePdf() async {
    ...
    final subTitle = pw.Align(
        alignment: pw.Alignment.topCenter,
        child: pw.Text(
          'Returned Product To Warehouse Report',
          textAlign: pw.TextAlign.center,
          style: pw.TextStyle(
            fontSize: 18,
            fontWeight: pw.FontWeight.bold,
...
// Footer widget for session information (SessionController number and name)
    ...
    for (int i = 0;
        i <
            controller.returnProductToWarehouseList.value.body!.returnProducts!
                .length;
        i++) {
      final returnedProduct = controller
          .returnProductToWarehouseList.value.body!.returnProducts![i];
      rows.add([
        "${i + 1}",

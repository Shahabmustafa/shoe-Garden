...
class PdfService {
  ...
  Future<Uint8List> printCustomersPdf() async {
    ...
    for (int i = 0;
        i < controller.customerList.value.body!.customers!.length;
        i++) {
      final customer = controller.customerList.value.body!.customers![i];
      rows.add([
        "${i + 1}",
        (customer.name ?? 'Null'),
        (customer.email ?? 'Null'),
        ...
        (customer.openingBalance.toString() ?? 'Null'),
        (customer.currentDate ?? 'Null'),
      ]);
    }
...
// Split rows into pages
  ...
  Future<void> printPdf() async {
    // Generate the PDF
    final Uint8List bytes = await printCustomersPdf();

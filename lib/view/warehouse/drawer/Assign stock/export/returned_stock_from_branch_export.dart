...
import '../../../../../viewmodel/warehouse/Assign Stock/returned_stock_from_branch_viewmodel.dart';

class ReturnedStockFromBranchExport {
  ...
  Future<Uint8List> generatePdf() async {
    ...
    final subTitle = pw.Align(
        alignment: pw.Alignment.topCenter,
        child: pw.Text(
          'Returned Stock From Branch Report',
          textAlign: pw.TextAlign.center,
          style: pw.TextStyle(
            fontSize: 18,
            fontWeight: pw.FontWeight.bold,
...
// Footer widget for session information (SessionController number and name)
    ...
    for (int i = 0;
        i < controller.returnedStockList.value.body!.returnProducts!.length;
        i++) {
      final stock = controller.returnedStockList.value.body!.returnProducts![i];
      rows.add([
        "${i + 1}",
        (stock.returnTo!.name ?? 'Null'),

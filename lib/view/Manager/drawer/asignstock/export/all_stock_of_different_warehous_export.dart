...
class AllStockOfDifferentWarehousExport {
  final controller = Get.put(AllStockOfWarehouseViewModel());
  ...
  Future<Uint8List> generatePdf() async {
    ...
    final subTitle = pw.Align(
        alignment: pw.Alignment.topCenter,
        child: pw.Text(
          'All Stock of Different Warehouse Report',
          textAlign: pw.TextAlign.center,
          style: pw.TextStyle(
            fontSize: 18,
            fontWeight: pw.FontWeight.bold,
...
// Footer widget for session information (SessionController number and name)
    ...
    for (int i = 0;
        i <
            controller
                .assignStockWarehouseList.value.body!.warehouseStocks!.length;
        i++) {
      final stock =
          controller.assignStockWarehouseList.value.body!.warehouseStocks![i];
      rows.add([
        "${i + 1}",

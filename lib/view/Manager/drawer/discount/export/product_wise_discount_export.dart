...
import '../../../../../viewmodel/admin/discount/product_wise_discount_viewmodel.dart';
import '../../../../../viewmodel/user_preference/session_controller.dart';

class ProductWiseDiscountExport {
...
// Footer widget for session information (SessionController number and name)
    ...
    for (int i = 0;
        i <
            controller
                .productWiseDiscountList.value.body!.productsDiscount!.length;
        i++) {
      final discount =
          controller.productWiseDiscountList.value.body!.productsDiscount![i];
      rows.add([
        "${i + 1}",
        (discount.product?.name ?? 'Null'),
        (discount.product?.salePrice.toString() ?? 'Null'),
        (discount.product?.purchasePrice.toString() ?? 'Null'),

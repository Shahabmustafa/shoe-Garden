...
import '../../../../../viewmodel/admin/company/company_viewmodel.dart';
...
// Footer widget for session information (SessionController number and name)
    ...
    for (int i = 0;
        i < controller.companyList.value.body!.companies!.length;
        i++) {
      final company = controller.companyList.value.body!.companies![i];
      rows.add([
        "${i + 1}",
        (company.name ?? 'Null'),
        (company.email ?? 'Null'),

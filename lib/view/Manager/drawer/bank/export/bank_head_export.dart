...
import '../../../../../viewmodel/admin/bank/bank_head_viewmodel.dart';
import '../../../../../viewmodel/user_preference/session_controller.dart';

class BankHeadExport {
...
// Footer widget for session information (SessionController number and name)
    ...
    for (int i = 0;
        i < controller.bankHeadList.value.body!.bankHeads!.length;
        i++) {
      final bank = controller.bankHeadList.value.body!.bankHeads![i];
      rows.add([
        "${i + 1}",
        (bank.name ?? 'Null'),
        (bank.accountNumber ?? 'Null'),
        (bank.balance.toString() ?? 'Null'),
      ]);
    }

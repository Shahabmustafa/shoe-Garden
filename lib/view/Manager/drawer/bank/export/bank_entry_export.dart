...
import '../../../../../viewmodel/admin/bank/bank_entry_viewmodel.dart';
import '../../../../../viewmodel/user_preference/session_controller.dart';

class BankEntryExport {
...
// Footer widget for session information (SessionController number and name)
    ...
    for (int i = 0;
        i < controller.bankEntryList.value.body!.bankEnteries!.length;
        i++) {
      final bank = controller.bankEntryList.value.body!.bankEnteries![i];
      rows.add([
        "${i + 1}",
        (bank.bankHead?.name ?? 'Null'),
        (bank.type == 0 ? 'Cash' : 'Online'),
        (bank.description ?? 'Null'),
        (bank.deposit.toString() ?? 'Null'),
        (bank.withdraw.toString() ?? 'Null'),
        (bank.date ?? 'Null'),
      ]);
    }

...
class PurchaseService {
  Box<PurchaseInvoiceLocalStorageModel>? _invoiceBox;

  Future<void> openBox() async {
    _invoiceBox = await Hive.openBox<PurchaseInvoiceLocalStorageModel>(
        'purchase_invoice');
  }
  ...
  Future<void> addOrUpdateInvoice(
      PurchaseInvoiceLocalStorageModel newInvoice) async {
    if (_invoiceBox == null) {
      await openBox();
    }
    ...
    if (existingInvoiceIndex != -1) {
      PurchaseInvoiceLocalStorageModel? existingInvoice =
      ...
      if (existingInvoice != null) {
        ...
        existingInvoice.companyId = newInvoice.companyId;
  ...
  Future<List<PurchaseInvoiceLocalStorageModel>> getInvoices() async {
    if (_invoiceBox == null) {
      await openBox();
    }
  ...
  Future<void> updateInvoice(
      int index, PurchaseInvoiceLocalStorageModel invoice) async {
    if (_invoiceBox == null) {
      await openBox();
    }
  ...
  Future<PurchaseInvoiceLocalStorageModel?> createNewInvoice(
      String invoiceNumber) async {
    if (_invoiceBox == null) {
      await openBox();
    }
    ...
    try {
      ...
      var newInvoice = PurchaseInvoiceLocalStorageModel(

...
import '../../branch/saleinvoice/database/model/sale_invoice_model.dart';
import '../../branch/saleinvoice/database/services.dart';

class WarehouseSaleInvoiceViewmodel extends GetxController {
  ...
  final Rx<SaleInvoiceModelLocalStorage> fetchedInvoice =
      SaleInvoiceModelLocalStorage(customerId: '').obs;
  ...
  Future<void> addSaleInvoice() async {
    try {
      loading.value = true;
      fetchedInvoice.value.saleMenId = warehouseId.value;
      fetchedInvoice.value.receivedAmount = recievedAmount.value.toString();
  ...
  Future<void> getSize() async {
    ...
    await _api.getSpecific({
      ...
      totalQuantityController.value.text = value.body!.totalQuantity.toString();

      discountController.value.text = value.body!.product!.discount.toString();
    }).catchError((error) {
    ...
    try {
      if (selectProduct.value.isNotEmpty &&
        ...
        SaleInvoiceModelLocalStorage newInvoice = SaleInvoiceModelLocalStorage(
          ...
          saleMenId: warehouseId.value.toString(),

...
import 'package:sofi_shoes/date/repository/admin/assingStock/search_dynamic_product_stock_rep.dart';
import 'package:sofi_shoes/date/repository/admin/product/product_repository.dart';

import '../../../date/repository/admin/assingStock/assign_stock_wareshouse_rep.dart';
import '../../../date/repository/admin/status/status_rep.dart';
import '../../../date/repository/admin/warehouse/warehouse_rep.dart';
import '../../../date/response/status.dart';
import '../../../model/admin/assign_stock_wareshouse_model.dart';
import '../../../model/admin/search_dynamic_product_stock_model.dart';
import '../../../res/utils/utils.dart';

class AssignStockWarehouseViewModel extends GetxController {
  ...
  /// repository
  final _api = AssignStockWarehouseRepository();
  final _statusApi = StatusRepository();
  final _wareHouseApi = WarehouseRepository();
  final _productApi = ProductRepository();
  final companyApi = SearchDynamicRepository();

  RxBool loading = false.obs;
  RxString searchValue = ''.obs;

  final rxRequestStatus = Status.LOADING.obs;
  final assignStockWaresHouseList = AAssignStockTwoWarehouseModel().obs;
  ...
  void setAssignStockList(AAssignStockTwoWarehouseModel value) =>
      assignStockWaresHouseList.value = value;
    ...
    try {
      ...
      _api.add(data).then((value) {
        ...
        if (value['success'] == true && value['error'] == null) {
          getAllWareHouse();
          Get.back();
          Utils.SuccessToastMessage('Add Assign Stock To Warehouse');
  ...
  updateWarehouse(String id) async {
    ...
    await _api.update(data, id,).then((value) {
      if (value['success'] == true && value['error'] == null) {
        Utils.SuccessToastMessage('Assign Stock to Warehouse is Successfully Update');
        getAllWareHouse();
        clear();
        Get.back();
  ...
  deleteWarehouse(String id) {
    _api.delete(id).then((value) {
      if (value['success'] == true && value['error'] == null) {
        Utils.SuccessToastMessage("Sucessfully Delete Assign Stock to Warehouse");
        getAllWareHouse();
        Get.back();
        clear();
      } else {
  ...
  productDropDown() {
    _productApi.getAllProduct().then((value) {
      for (var entry in value.body!.products!) {
        ...
        print(dropdownItemsProduct.length);
      }
    }).catchError((error) {
  ...
  totalStock() {
    companyApi.getSpecific({
      ...
      print('Error Fetching Total Stock Name: $error');
      // Set loading state to false
    });
  }
